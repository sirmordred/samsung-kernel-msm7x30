...
static u16 intensity_data[176] = { 0, };
...
static void check_intensity_data(struct mcs8000_ts_driver *ts)
{
    ...
    for(i = 0; i < sensing_ch; i++ ) {
        ...
        for( j = 0; j <  read_data_len; j++)
            intensity_data[(i * exciting_ch) + j] = read_data_buf[j];

    }

    printk("[TSP] intensity_data");
	
	for (i = 0; i < 11*16; i++) {
		if (0 == i % 16)
			printk("\n");
		printk("0x%02x, ", intensity_data[i]);
    	}
    printk("\n"); 
